...
class TRQAMAgent(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        return actor_loss + total_fast_loss, {
            'flow_loss': flow_loss, "fast_loss": total_fast_loss, **info}
    ...
    def sample_actions(
        ...
        action_dim = self.config['action_dim'] * \
                        (self.config['horizon_length'] if self.config["action_chunking"] else 1)
        bfns = self.config["q_bfn"] if use_q_bfn else self.config["best_of_n"]
        ...
        observations = jnp.repeat(observations[..., None, :], bfns, axis=-2)
        ...
        bsize = len(indices)
        actions = jnp.reshape(actions, (-1, bfns, action_dim))[jnp.arange(bsize), indices, :].reshape(
            bshape + (action_dim,))
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            action_chunking=True,                                      # Use Q-chunking or just n-step return
