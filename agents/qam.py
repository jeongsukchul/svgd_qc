...
class QAMAgent(flax.struct.PyTreeNode):
    ...
    def adj_matching(self, obs, rng, flow_steps=None):
        ...
        if self.config["clip_adj"]:
            ...
            grad_fn = jax.grad(
                lambda x, y: self.network.select(critic_network)(x, y).mean(axis=0).sum(),
                1,
            )
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        if self.config["residual"]:
            ...
            adj_loss = jnp.sum(
                jnp.square((vf_fine - vf_base) * 2 / sigmas + sigmas * adjs),
                axis=-1,
            )
        ...
        if config["edit_scale"] > 0.0:
            ...
            network_info.update(
                dict(
                    edit_actor=(
                        edit_actor_def,
                        jnp.concatenate((ex_observations, full_actions), axis=-1),
                    ),
