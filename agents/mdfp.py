...
class MDFPAgent(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        rng, noise_rng, output_noise_rng = jax.random.split(rng, 3)
        ...
        generated_actions = self.network.select('actor_drift')(
            obs_repeated, noises, params=grad_params
        )
        generated_actions = self._add_actor_output_noise(
            generated_actions, output_noise_rng
        ).reshape(batch_size, self.config['num_generations'], action_dim)
    ...
    def _pretrain_bc_update(agent, batch):
        """Update only the drift behavior model before offline RL."""
        new_rng, rng = jax.random.split(agent.rng)
    ...
    def _update_frozen_bc(agent, batch):
        """Update the agent while keeping the drift behavior model fixed."""
        new_rng, rng = jax.random.split(agent.rng)
