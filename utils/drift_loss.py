...
def drift_loss(
    ...
    if use_neg_only:
        # Negative side uses only fixed_neg; gen samples are not implicit negatives.
    ...
    def calculate_scaled_goal_and_factor(old_gen_in, targets_in, targets_w_in):
        ...
        # --- Masking (only when gen samples are part of targets) ---
        if not use_neg_only:
            mask_val = 100.0
        ...
        for R in R_list:
            ...
            R_coeff = jnp.concatenate([r_coeff_neg, r_coeff_pos], axis=2) # [B, C_g, C_n + C_p]
