...
class Dataset(FrozenDict):
    ...
    def sample_sequence(self, batch_size, sequence_length, discount):
        ...
        action_shape = data['actions'].shape
        ...
        observations = np.zeros(
            obs_shape[:-1] + (sequence_length, obs_shape[-1]),
            dtype=data['observations'].dtype,
        )
        next_observations = np.zeros_like(observations)
        actions = np.zeros(
            action_shape[:-1] + (sequence_length, action_shape[-1]),
            dtype=data['actions'].dtype,
        )
        next_actions = np.zeros_like(actions)

        def expand_mask(mask, target):
            return np.reshape(mask, mask.shape + (1,) * (target.ndim - mask.ndim))

        def scalar_values(values):
            return np.asarray(values).reshape(batch_size, -1)[:, 0]

        for i in range(sequence_length):
            cur_idxs = idxs + i

            cur_rewards = scalar_values(self['rewards'][cur_idxs])
            cur_masks = scalar_values(self['masks'][cur_idxs])
            cur_terminals = scalar_values(self['terminals'][cur_idxs])
            cur_next_observations = self['next_observations'][cur_idxs]

            if i == 0:
                rewards[..., i] = cur_rewards
                masks[..., i] = cur_masks
                terminals[..., i] = cur_terminals
            else:
                valid[..., i] = 1.0 - terminals[..., i - 1]
                rewards[..., i] = rewards[..., i - 1] + (
                    cur_rewards * (discount ** i) * valid[..., i]
                )
                masks[..., i] = (
                    np.minimum(masks[..., i - 1], cur_masks) * valid[..., i]
                    + masks[..., i - 1] * (1.0 - valid[..., i])
                )
                terminals[..., i] = np.maximum(terminals[..., i - 1], cur_terminals)

            valid_obs = expand_mask(valid[..., i], cur_next_observations)
            observations[..., i, :] = self['observations'][cur_idxs]
            next_observations[..., i, :] = (
                cur_next_observations * valid_obs
                + next_observations[..., i - 1, :] * (1.0 - valid_obs)
            )
            actions[..., i, :] = self['actions'][cur_idxs]

            next_action_idxs = np.minimum(cur_idxs + 1, self.size - 1)
            cur_next_actions = self['actions'][next_action_idxs]
            valid_actions = expand_mask(valid[..., i], cur_next_actions)
            next_actions[..., i, :] = (
                cur_next_actions * valid_actions
                + next_actions[..., i - 1, :] * (1.0 - valid_actions)
            )
